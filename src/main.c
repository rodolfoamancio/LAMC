...
void RunSimulation(char* InputFileName){
  ...
  for(int i=1; i<=NumberTotalSteps + 1; i++){
    ...
    if(i%StepsGetProfiles==0 && SimulationBox.ClosedBox && ChainSize > 1 && StepsGetProfiles>0){
    ...
    if(i%StepsCalculateProperties==0){
      ...
      fprintf(
        ...
        i,
        GetEnsembleLabel(Ensemble),
        ProductionFlag,
        DisplacementAcceptanceCalculated,
        MaxTranslationDistance,
        Temperature,
        SimulationBox.xSize,
        SimulationBox.ySize,
        SimulationBox.zSize,
        OldConfiguration.NumberMolecules,
        DensityMass,
        DensityMolar,
        PotentialTotal,
        PotentialBonded,
        PotentialNonbonded,
        PotentialLongRangeCorrection,
        PotentialWalls,
        PotentialPerturbed,
        WeightIdealChain,
        STDWeightIdealChain,
    ...
    if(i%OptimizeAcceptanceEverySteps==0 && i<NumberEquilibrationSteps){
      ...
      DisplacementAcceptanceRatio = LimMinMax(DisplacementAcceptanceRatio, 0.5, 1.5);
      MaxTranslationDistance *= DisplacementAcceptanceRatio;
      MaxTranslationDistance = LimMinMax(MaxTranslationDistance, 0.05, 8.0);
    }
